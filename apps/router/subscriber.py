"""
Approved Order Subscriber

Subscribes to approved OrderIntent messages from risk engine.
"""
...
class ApprovedOrderSubscriber:
    """
    Subscriber for approved order intents.
    
    Receives OrderIntent messages that passed risk checks.
    """
    
    def __init__(self, config, routing_engine):
        self.config = config
    ...
    async def start(self):
        ...
        self.logger.info("approved_order_subscriber_started", transport=self.config.ipc.transport)
    ...
    async def stop(self):
        ...
        self.logger.info("approved_order_subscriber_stopped")
    ...
    async def _start_zmq(self):
        ...
        # Connect to risk engine approved orders
        port = self.config.ipc.zmq_base_port + 20 + 1  # Risk publishes on +1
        connect_addr = f"tcp://127.0.0.1:{port}"
        self.socket.connect(connect_addr)
        
        # Subscribe to approved topic
        self.socket.setsockopt(zmq.SUBSCRIBE, b"approved")
        
        self.logger.info("zmq_approved_order_subscriber_connected", address=connect_addr)
        self.running = True
    
    async def _receive_loop(self):
        """Receive loop for processing approved orders"""
        self.logger.info("approved_order_receive_loop_started")
        ...
        while self.running:
            try:
                ...
                if order:
                    ...
                    # Record metrics
                    self.metrics.increment_received("OrderApproved")
                    self.metrics.record_latency("approved_receive", latency_us)
                    
                    # Route the order
                    await self.routing_engine.route_order(order)
        ...
        self.logger.info("approved_order_receive_loop_stopped")
