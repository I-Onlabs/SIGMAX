"""
Order Intent Subscriber

Subscribes to OrderIntent messages from decision layer (stream 20).
"""
...
class OrderSubscriber:
    """
    Subscriber for order intents.
    
    Receives OrderIntent messages from stream 20.
    """
    ...
    async def start(self):
        ...
        self.logger.info("order_subscriber_started", transport=self.config.ipc.transport)
    ...
    async def stop(self):
        ...
        self.logger.info("order_subscriber_stopped")
    ...
    async def _start_zmq(self):
        ...
        # Connect to stream 20 (orders)
        port = self.config.ipc.zmq_base_port + 20
        connect_addr = f"tcp://127.0.0.1:{port}"
        self.socket.connect(connect_addr)
        
        # Subscribe to orders topic
        self.socket.setsockopt(zmq.SUBSCRIBE, b"orders")
        
        self.logger.info("zmq_order_subscriber_connected", address=connect_addr)
        self.running = True
    
    async def _receive_loop(self):
        """Receive loop for processing orders"""
        self.logger.info("order_receive_loop_started")
        ...
        while self.running:
            try:
                ...
                if order:
                    ...
                    # Record metrics
                    self.metrics.increment_received("OrderIntent")
                    self.metrics.record_latency("order_receive", latency_us)
                    
                    # Process in risk engine
                    await self.risk_engine.check_order(order)
        ...
        self.logger.info("order_receive_loop_stopped")
